...
if (!userRepo.Exists(adminEmail))
{
    ...
    userRepo.Add(admin);
    logger.Info($"Создан администратор: {adminEmail} / {adminPassword}");
...
if (!userRepo.Exists(moderatorEmail))
{
    ...
    logger.Info($"Создан модератор: {moderatorEmail} / {moderatorPassword}");
...
if (!userRepo.Exists(sellerEmail))
{
    ...
    userRepo.Add(seller);
    logger.Info($"Создан продавец: {sellerEmail} / {sellerPassword}");
...
// СОЗДАНИЕ ПОКУПАТЕЛЯ (для тестов)
...
if (!userRepo.Exists(buyerEmail))
{
    ...
    userRepo.Add(buyer);
    logger.Info($"Создан покупатель: {buyerEmail} / {buyerPassword}");
